...
col_factorsO <- c(names(df1[24:29]),'Medu','Fedu','traveltime','studytime')
...
barplot(table(df1$school))
barplot(table(df1$Medu))
barplot(table(df1$Fedu))
barplot(table(df1$Mjob))
barplot(table(df1$Fjob))
barplot(table(df1$Pstatus))
barplot(table(df1$traveltime))
barplot(table(df1$studytime))
barplot(table(df1$goout))
```
